  ...
  for (int i = 0; i < 8; i++)
  {
    for (int j = 0; j < 14; j++)
    {
      ...
      for (int Q = 0; Q < 8; Q++)
      {
        int q = Q_1[Q];
        int qq = Q_2[Q];
        if (!(0 <= q && q < 8 && 0 <= qq && qq < 14 && x[q][qq] == s[1]))
          continue;
        if (sl == 2)
          return true;
        ...
        for (int W = 0; W < 8; W++)
        {
          int w = W_1[W];
          int ww = W_2[W];
          if (!(0 <= w && w < 8 && 0 <= ww && ww < 14 && x[w][ww] == s[2]))
            continue;
          if (sl == 3)
            return true;
          ...
          for (int E = 0; E < 8; E++)
          {
            int e = E_1[E];
            int ee = E_2[E];
            if (!(0 <= e && e < 8 && 0 <= ee && ee < 14 && x[e][ee] == s[3]))
              continue;
            return true;
          }
        }
...
int main(int argc, char *args[])
{
  ...
  for (int loopcount = 0; loopcount < count; loopcount++)
  {
    ...
    if (loop)
    {
      ...
      closedir(d);
      filenames[count] = score;
      count++;
      qsort(filenames, count, sizeof(int), compare);
      bool isBest = true; // true면 저장해야함
      for (int i = 0; i < count - 5; i++)
      {
        if (filenames[i] == score)
        {
          // printf("hmm\n");
          isBest = false;
          continue;
